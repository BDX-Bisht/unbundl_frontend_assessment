...
export default function ResultsAndWhy() {
  ...
  const fetchWhyData = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('https://jsonplaceholder.typicode.com/posts?_start=10&_limit=4');
      ...
      const mappedWhyData = data.map((post, idx) => {
        ...
        const cleanTitle = headings[idx] || (post.title.split(' ').slice(0, 3).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '));
        const cleanDesc = post.body.split('.').slice(0, 2).join('.') + '.';
      ...
      <div className="why-section">
        ...
        {error && (
          <div className="why-error-container">
            <p className="why-error-text">Failed to load Whistle benefits. Please try again.</p>
            <button type="button" onClick={fetchWhyData} className="why-retry-btn">
              Retry
            </button>
          </div>
        )}
